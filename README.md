...
**Validation pass** (`data/scan-results/memory-mapping-validation-2026-06-21.json`): re-ran the chain detection across all 3 independent captures (taken hours apart) and kept only chains whose boundaries are byte-for-byte identical every time - **52 stable chains**, strong evidence these are real structural edges, not a one-capture artifact. Separately, checked whether the **5 known sensor PIDs** connect to either neighbor the same way the bitfield groups do - **none of them do** (the one apparent match, `1113`'s neighbor, is a trivial zero-coincidence, not a real overlap). That's a genuine structural distinction: the sensor PIDs appear to be **individually-addressed standalone cells**, while the bitfield status groups are **packed into contiguous memory** - probably two different internal table layouts.

**9 of the 50 still-unexplained stable chains contain a value that changed** between the first and third capture (hours apart) - mostly a single live byte followed by a run of zero padding, with small bounded deltas (`+3`, `+32`, `-12`, `-26`, etc., never a big jump), which looks more like a real analog-ish sensor than a counter/timer. These are now the priority list for future decode work, full data in the validation file above: `1088`,`10A0`,`1164`,`11EF`,`11F8`,`1248`,`1278`,`12A8`,`12B8`.
